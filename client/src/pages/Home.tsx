...
export default function Home() {
  ...
  const selectedLender = useMemo(() => dashboard.data?.lenders.find(lender => lender.id === selectedLenderId) ?? null, [dashboard.data?.lenders, selectedLenderId]);
  const visibleProducts = useMemo(() => products.data?.filter(product => selectedLenderId === null || product.lenderId === selectedLenderId) ?? [], [products.data, selectedLenderId]);
    ...
    <div className="mx-auto max-w-[1560px] space-y-7 pb-12" id="overview">
      ...
      <section className="grid gap-5 xl:grid-cols-[1.65fr_0.85fr]" id="lenders">
        <Card className="overflow-hidden border-white/80 bg-card/90 shadow-sm">
          ...
          <CardContent className="p-0"><div className="max-h-[520px] overflow-auto"><table className="w-full min-w-[900px] text-left text-sm"><thead className="sticky top-0 z-10 bg-slate-50 text-xs uppercase tracking-[0.1em] text-slate-500"><tr><th className="px-5 py-3 font-medium">Lender</th><th className="px-5 py-3 font-medium">Website</th><th className="px-5 py-3 font-medium">Product page</th><th className="px-5 py-3 font-medium">Last scrape</th><th className="px-5 py-3 font-medium">Status</th><th className="px-5 py-3" /></tr></thead><tbody className="divide-y divide-slate-100">{dashboard.data?.lenders.map(lender => <tr key={lender.id} className="group hover:bg-teal-50/35"><td className="px-5 py-4 font-medium text-slate-800">{lender.name}<div className="mt-1 text-xs font-normal text-muted-foreground">{lender.productCount} active products</div>{lender.lastErrorMessage && <div className="mt-1 max-w-[230px] truncate text-xs font-normal text-rose-700" title={lender.lastErrorMessage}>{lender.lastErrorMessage}</div>}</td><td className="px-5 py-4"><a className="inline-flex max-w-[190px] items-center gap-1 truncate text-xs text-slate-500 hover:text-primary" href={lender.mainWebsiteUrl ?? undefined} target="_blank" rel="noreferrer">{lender.mainWebsiteUrl ? new URL(lender.mainWebsiteUrl).hostname : "—"}<ArrowUpRight className="h-3 w-3 shrink-0" /></a></td><td className="px-5 py-4"><a className="inline-flex max-w-[210px] items-center gap-1 truncate text-xs text-primary hover:underline" href={lender.productPageUrl ?? undefined} target="_blank" rel="noreferrer">{lender.productPageUrl ? "Product source" : "Not mapped"}<ArrowUpRight className="h-3 w-3 shrink-0" /></a></td><td className="px-5 py-4 text-xs text-muted-foreground">{dateLabel(lender.lastScrapedAt)}</td><td className="px-5 py-4"><Badge variant="outline" className={`capitalize ${statusTone[lender.scrapeStatus]}`}>{lender.lastErrorCategory ?? lender.scrapeStatus}</Badge></td><td className="flex justify-end gap-1 px-5 py-4"><Button size="sm" variant="ghost" title="Show this lender's results" onClick={() => setSelectedLenderId(lender.id)}><TableProperties className="h-3.5 w-3.5" /></Button><Button size="sm" variant="ghost" title="Download this lender's JSON" onClick={() => { const lenderProducts = products.data?.filter(product => product.lenderId === lender.id) ?? []; downloadFile(`${lender.normalizedName || "lender"}-result.json`, JSON.stringify({ lender: { id: lender.id, name: lender.name, sourceUrl: lender.productPageUrl ?? lender.mainWebsiteUrl, scrapeStatus: lender.scrapeStatus, errorCategory: lender.lastErrorCategory, errorMessage: lender.lastErrorMessage, lastScrapedAt: lender.lastScrapedAt }, products: lenderProducts }, null, 2), "application/json"); toast.success(`${lender.name} result JSON downloaded.`); }}><Download className="h-3.5 w-3.5" /></Button><Button size="sm" variant="ghost" title="Download this lender's workbook" onClick={() => exportWorkbook.mutate({ lenderId: lender.id })} disabled={exportWorkbook.isPending}><FileSpreadsheet className="h-3.5 w-3.5" /></Button><Button size="sm" variant={lender.scrapeStatus === "failed" ? "outline" : "ghost"} onClick={() => run.mutate({ lenderId: lender.id, trigger: lender.scrapeStatus === "failed" ? "retry" : "manual" })} disabled={busy}>{lender.scrapeStatus === "failed" ? <><RotateCcw className="mr-1.5 h-3.5 w-3.5" />Retry</> : <><Play className="mr-1.5 h-3.5 w-3.5" />Run</>}</Button></td></tr>)}{!dashboard.data?.lenders.length && <tr><td colSpan={6} className="px-5 py-14 text-center text-sm text-muted-foreground">Sync the supplied Google Sheets to load the lender queue.</td></tr>}</tbody></table></div></CardContent>
        </Card>
      ...
      <section className="grid gap-5 xl:grid-cols-[1.65fr_0.85fr]" id="review">
        <Card className="border-white/80 bg-card/90 shadow-sm"><CardHeader className="flex-row items-center justify-between gap-3 border-b bg-white/40 py-5"><div><CardTitle className="text-lg">{selectedLender ? `${selectedLender.name} results` : "Product review queue"}</CardTitle><CardDescription>{selectedLender ? "Only this lender’s extracted records are shown and downloaded." : "Choose a lender from the capture ledger to review its records separately."}</CardDescription></div><div className="flex items-center gap-2"><Select value={selectedLenderId?.toString() ?? "all"} onValueChange={value => setSelectedLenderId(value === "all" ? null : Number(value))}><SelectTrigger className="w-[185px]"><SelectValue placeholder="All lenders" /></SelectTrigger><SelectContent><SelectItem value="all">All lenders</SelectItem>{dashboard.data?.lenders.map(lender => <SelectItem key={lender.id} value={String(lender.id)}>{lender.name}</SelectItem>)}</SelectContent></Select><Button size="sm" variant="outline" onClick={() => products.refetch()}><RefreshCw className="mr-2 h-4 w-4" /> Update</Button></div></CardHeader><CardContent className="p-0"><div className="max-h-[430px] overflow-auto"><table className="w-full min-w-[840px] text-left text-sm"><thead className="sticky top-0 bg-slate-50 text-xs uppercase tracking-[0.1em] text-slate-500"><tr><th className="px-5 py-3 font-medium">Product</th><th className="px-5 py-3 font-medium">Lender</th><th className="px-5 py-3 font-medium">Rate / LTV</th><th className="px-5 py-3 font-medium">Lifecycle</th><th className="px-5 py-3 font-medium">Confidence</th><th className="px-5 py-3" /></tr></thead><tbody className="divide-y divide-slate-100">{visibleProducts.map(product => <tr key={product.id} className="hover:bg-teal-50/35"><td className="px-5 py-4 font-medium">{product.data.product}<div className="mt-1 text-xs font-normal text-muted-foreground">{product.data.purpose ?? "Purpose not supplied"}</div></td><td className="px-5 py-4 text-sm text-slate-600">{product.lenderName}</td><td className="px-5 py-4 text-sm text-slate-600">{product.data.rate !== null ? `${(product.data.rate * 100).toFixed(2)}%` : "—"} <span className="text-muted-foreground">/</span> {product.data.maxLtv !== null ? `${(product.data.maxLtv * 100).toFixed(0)}%` : "—"}</td><td className="px-5 py-4"><Badge variant="outline" className="capitalize">{product.lifecycle}</Badge></td><td className="px-5 py-4"><span className="font-medium text-slate-700">{Math.round(Number(product.confidence) * 100)}%</span></td><td className="px-5 py-4 text-right"><Button size="sm" variant="ghost" onClick={() => { setReviewId(product.id); setReviewJson(JSON.stringify(product.data, null, 2)); }}>Review</Button></td></tr>)}{!visibleProducts.length && <tr><td colSpan={6} className="px-5 py-12 text-center text-sm text-muted-foreground">{selectedLender ? "No product records were captured for this lender. Its blocked or failed status remains visible in the capture ledger." : "Product rows appear here after a successful browser capture."}</td></tr>}</tbody></table></div></CardContent></Card>
        <div className="space-y-5">
          <Card className="border-white/80 bg-card/90 shadow-sm"><CardHeader className="pb-3"><CardTitle className="text-base">Export centre</CardTitle><CardDescription>{selectedLender ? `Download ${selectedLender.name} separately.` : "Select a lender to download only that lender’s data, or export the complete portfolio."}</CardDescription></CardHeader><CardContent className="space-y-3"><Button className="w-full justify-start" onClick={() => exportWorkbook.mutate(selectedLender ? { lenderId: selectedLender.id } : undefined)} disabled={exportWorkbook.isPending}><FileSpreadsheet className="mr-2 h-4 w-4" /> {exportWorkbook.isPending ? "Building reference workbook…" : selectedLender ? "Download this lender’s workbook" : "Export all lenders’ workbook"}</Button><Button className="w-full justify-start" variant="outline" onClick={() => { const content = selectedLender ? { lender: { id: selectedLender.id, name: selectedLender.name, sourceUrl: selectedLender.productPageUrl ?? selectedLender.mainWebsiteUrl, scrapeStatus: selectedLender.scrapeStatus, errorCategory: selectedLender.lastErrorCategory, errorMessage: selectedLender.lastErrorMessage, lastScrapedAt: selectedLender.lastScrapedAt }, products: visibleProducts } : visibleProducts; downloadFile(`${selectedLender?.normalizedName ?? "lender"}-${selectedLender ? "result" : "products"}.json`, JSON.stringify(content, null, 2), "application/json"); toast.success(selectedLender ? `${selectedLender.name} result JSON downloaded.` : "All lender JSON downloaded."); }}><Download className="mr-2 h-4 w-4" /> {selectedLender ? "Download this lender’s JSON" : "Download all lenders’ JSON"}</Button><div className="rounded-xl bg-slate-50 p-3 text-xs leading-5 text-muted-foreground">The exporter starts with your supplied workbook template and preserves its Introduction, Current Products, New Products, Withdrawn Products, and Additional sheets.</div></CardContent></Card>
